		...
		if( gin[0].Age > -0.1f )
		{
			for(int i = 0; i < 2; ++i)
			{
				for(int j = 0; j < 45; ++j)
				{
					...
					float3 vRandom = 100.0f*RandUnitVec3((float)i*j/45.0f);//*((float)i/45.0f);
					//vRandom.x *= 0.5f*((float)i/45.0f);
					//vRandom.y *= 0.5f*((float)i/45.0f);
					//vRandom.z *= 0.5f*((float)i/45.0f);
									
					//vRandom.x += i*2 + cos(gin[0].Age*2)*400;					
					//vRandom.y += j*2 ;
					//vRandom.z = sin(gin[0].Age*2)*gin[0].Age;

					vRandom.x = j*2;					
					vRandom.y = i*2 ;
					vRandom.z = gin[0].Age*2;

					Particle p;
					p.InitialPosW = gEmitPosW.xyz + vRandom;//+RandUnitVec3((float)i/100.0f);
					p.InitialVelW = 0.0f; //1000.0f*vRandom *0.1f;//*((float)i/100.0f);
					p.SizeW       = float2(10.0f, 10.0f);
					p.Age         = 0.0f;
					p.Type        = PT_FLARE;
					p.noParticles = gin[0].noParticles + 90;
...
VertexOut DrawVS(Particle vin)
{
	VertexOut vout;
	float t = vin.Age;
	float3 accel = {sin(vin.Age)*0.0f,sin(vin.Age), cos(vin.Age)*0.0f}; 
