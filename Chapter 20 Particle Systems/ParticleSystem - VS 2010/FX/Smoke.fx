...
void StreamOutGS(point Particle gin[1], 
                 inout PointStream<Particle> ptStream, inout PointStream<Particle> ptStream2)
{	
	gin[0].Age += gTimeStep;
	...
	{	
		...
		{			
			...
			gin[0].Age = 0.0f;
			gin[0].noOfParts = gin[0].noOfParts + 1;
		}
...
VertexOut DrawVS(Particle vin)
{
	VertexOut vout;
	float t = vin.Age;
	float3 accel = {sin(vin.Age)*1,0.0f, cos(vin.Age)*1}; 

	// constant acceleration equation
	vout.PosW = 0.5f*t*t*gAccelW  +0.5f*t*t*accel + t*vin.InitialVelW  + vin.InitialPosW;
