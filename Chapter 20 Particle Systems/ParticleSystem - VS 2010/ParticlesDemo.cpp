...
private:
	...
	//ParticleSystem mSmoke2;
	//ParticleSystem mSmoke3;
...
bool ParticlesApp::Init()
{
	...
	//flares.push_back(L"Textures\\smoke0.dds");
	//mFlareTexSRV = d3dHelper::CreateTexture2DArraySRV(md3dDevice, md3dImmediateContext, flares);

	//mFire.Init(md3dDevice, Effects::SmokeFX, mFlareTexSRV, mRandomTexSRV, 50000); 
	//mFire.SetEmitPos(XMFLOAT3(20.0f, 1.0f, 120.0f));

	std::vector<std::wstring> flare;
	flare.push_back(L"Textures\\smoke0.dds");
	//flare.push_back(L"Textures\\flare0.dds");
	mFlareTexSRV = d3dHelper::CreateTexture2DArraySRV(md3dDevice, md3dImmediateContext, flare);

	mSmoke.Init(md3dDevice, Effects::SmokeFX, mFlareTexSRV, mRandomTexSRV, 100000); 
	mSmoke.SetEmitPos(XMFLOAT3(0.0f, 1.0f, 120.0f));

	//mSmoke2.Init(md3dDevice, Effects::SmokeFX, mFlareTexSRV, mRandomTexSRV, 50000); 
	//mSmoke2.SetEmitPos(XMFLOAT3(-20.0f, 1.0f, 120.0f));
	//mSmoke3.Init(md3dDevice, Effects::SmokeFX, mFlareTexSRV, mRandomTexSRV, 50000); 
	//mSmoke3.SetEmitPos(XMFLOAT3(-40.0f, 1.0f, 120.0f));
...
void ParticlesApp::UpdateScene(float dt)
{
	...
	//mSmoke2.Update(dt, mTimer.TotalTime());
	//mSmoke3.Update(dt, mTimer.TotalTime());
...
void ParticlesApp::DrawScene()
{
	...
	//mSmoke2.SetEyePos(mCam.GetPosition());
	//mSmoke3.SetEyePos(mCam.GetPosition());
	mSmoke.Draw(md3dImmediateContext, mCam);	
	//mSmoke2.Draw(md3dImmediateContext, mCam);
	//mSmoke3.Draw(md3dImmediateContext, mCam);
